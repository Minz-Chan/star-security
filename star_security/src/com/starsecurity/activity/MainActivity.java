 ...
 * @description 修改记录 2013-12-04 修正新拍照片在相册无法即时更新问题 陈明珍 2013-12-02 修正光圈放大和光圈缩小提示相反
 *              陈明珍 2013-11-26 当返回的最大支持通道数为1时，点击除1外的其他通道，不响应点击事件且图标为灰色
 *              若请求的通道为当前正在播放的通道，则不响应切换请求。 肖远东 2013-11-22 添加“最大通道数为1，不允许切换通道功能”
 *              肖远东 2013-10-23 修正“向左拖动，出现全黑画面”、“保存图片成功后，做其他操作，一直显示保存图片提示。”、
 *              “光圈变大和缩小图标的提示相反”问题 陈明珍 2013-10-11 顶部原显示IP位置布局变更加入设备列表按钮 陈明珍
 *              2013-10-06 DDNS搜索列表点击设备后直接播放视频 陈明珍 2013-07-30
 *              使用UI消息接收机制处理非UI线程传递的界面更新消息(Handler) 陈明珍 2013-07-30
 *              网络连线中断后自动切换播放按钮并给出提示 陈明珍 2013-07-30 加入获取最后一次设置功能 肖远东 2013-07-27
 *              修正 BACK 键部分情况下出现异常 陈明珍 2013-07-25 修正屏幕分辨率适应问题以及Android
 *              4.2版本API兼容性问题 陈明珍 2013-06-08 加入断线重连功能（3分钟后重连） 陈明珍 2013-05-17
 *              加入控制停止动作controlService.stopAction 陈明珍 2013-05-07
 *              加入方向控制、放大/缩小、焦点放大/缩小、光圈放大/缩小功能 陈明珍 2013-05-03 加入提示信息对字符串资源ID的支持
 *              陈明珍 加入拍照功能 2013-05-03 ViewManager加入上下文初始化 陈明珍 2013-04-26
 *              当前Connection信息获取 陈明珍 ConnectionManager.getConnection("conn1")
...
public class MainActivity extends Activity {
	...
	private Button ws_listview_fav_btn;// 宽屏显示按钮
	...
	/** 使用记录单元实例 */
	private FavouriteControlService favouriteControlService = new FavouriteControlServiceImpl(
			"conn1");
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (ActivityUtility.getScreenSize(this).x > ActivityUtility
				.getScreenSize(this).y) {
			ActivityUtility.hideTitleBar(this);
			ViewManager.getInstance().setWideScreen(true);

			setContentView(R.layout.ws_activity_main);
			// ToastUtils.show(this, "status height: " +
			// String.valueOf(getStatusBarHeight(this)));
			// ToastUtils.show(this, "width: " +
		...
		if (isFirstStart()) {
			...
			if (myFavouritesFile.exists()) {
				try {
					...
					favouriteControlService.createFileAndRoot(filePath,
							"Favourites"); // 创建文件
				} catch (Exception e) {
			...
			// factory.inflate(R.layout.cloudsetting_dialog, null);
			...
			final EditText ddnsServerIP = (EditText) textEntryView
					.findViewById(R.id.ddnsEditTextServerIP);
			final EditText ddnsServerPort = (EditText) textEntryView
					.findViewById(R.id.ddnsEditTextServerPort);
			final EditText ddnsUserName = (EditText) textEntryView
					.findViewById(R.id.ddnsEditTextUserName);
			final EditText ddnsUserPassword = (EditText) textEntryView
					.findViewById(R.id.ddnsEditTextUserPassword);
			AlertDialog.Builder ad1 = new AlertDialog.Builder(MainActivity.this);
			ad1.setTitle(R.string.ddns_setting);
			ad1.setIcon(android.R.drawable.ic_dialog_info);
			ad1.setView(textEntryView);
			ad1.setPositiveButton(R.string.IDS_Btn_OK,
					new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog, int i) {
							if (ddnsServerIP.getText() != null) {
								ddnsServerIPString = ddnsServerIP.getText()
										.toString();
							}
							if (ddnsServerPort.getText() != null) {
								ddnsServerPortString = ddnsServerPort.getText()
										.toString();
							}
							if (ddnsUserName.getText() != null) {
								ddnsUserNameString = ddnsUserName.getText()
										.toString();
							}
							if (ddnsUserPassword.getText() != null) {
								ddnsPasswordString = ddnsUserPassword.getText()
										.toString();
							}
							favouriteControlService.setServerIP(filePath,
									ddnsServerIPString);
							favouriteControlService.setServerPort(filePath,
									ddnsServerPortString);
							favouriteControlService.setUserName(filePath,
									ddnsUserNameString);
							favouriteControlService.setPassword(filePath,
									ddnsPasswordString);
							...
							Toast.makeText(getApplicationContext(),
									R.string.IDS_RecordSaveSuc,
									Toast.LENGTH_LONG).show();
						}
					});
			ad1.setNegativeButton(R.string.IDS_Btn_Quit,
					new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog, int i) {
			...
			ddnsServerPortString = favouriteControlService
		...
		if (myFavouritesFile.exists()) {
			String selectedRecordName = favouriteControlService
					.getLastRecordName(filePath);
			String lastChannel = favouriteControlService
					.getLastChannel(filePath);
			if (selectedRecordName != null && !selectedRecordName.equals("")) {
				FavouriteRecord lastFavouriteRecord = (FavouriteRecord) favouriteControlService
						.getFavouriteRecordByName(filePath, selectedRecordName);
		 ...
		 * SearchableDeviceListActivity.class);
		 * 
		...
		if (ViewManager.getInstance().isWideScreen()) {
			ws_listview_fav_btn = (Button) findViewById(R.id.ws_listview_fav_btn);
			ws_listview_fav_btn.setOnClickListener(new OnClickListener() {

				@Override
				public void onClick(View v) {
					Intent intent = new Intent();
					intent.setClass(MainActivity.this,
							CollectDeviceActivity.class);
					startActivityForResult(intent, 0);
				}
			});
		}
			...
			public void onClick(View v) {
				if (!isPlay) {
					if (dvrDevice != null && isCloudControl) {
						...
						if (settingRecName != null
								&& !settingRecName.trim().equals("")) {
					...
					if (isWideScreen()) {
						playBtn.setBackgroundDrawable(getResources()
								.getDrawable(R.drawable.ws_linear_left));
					} else {
						playBtn.setBackgroundDrawable(getResources()
								.getDrawable(R.drawable.linear_left));
					}
		 ...
		 * 
		 * @Override public boolean onTouch(View v, MotionEvent event) { //单击事件
		 * if(event.getAction() == MotionEvent.ACTION_DOWN) { if(isPlay){
		 * functionTempStr = msgView.getText().toString();
		 * extendedService.takePicture(); } } //按钮松开时，显示播放参数
		 * if(event.getAction() == MotionEvent.ACTION_UP) { if(isPlay) {
		 * msgView.setText(functionTempStr); } } return false; } });
		 */
		...
		groupBtn.setOnClickListener(new Button.OnClickListener() {
			@Override
			public void onClick(View v) {
				TLV_V_DVSInfoRequest tlvDVSInfoRequest = (TLV_V_DVSInfoRequest) ConnectionManager
						.getConnection("conn1").retrieveResultItem(
								TLV_T_Command.TLV_T_DVS_INFO_REQUEST);
		...
		mHandler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				case MessageCode.IDS_CONNECTMAXCHANNEL:
					if (settingChannel == null
							|| settingChannel.trim().equals("")) {
						settingChannel = "1";
					}
					TLV_V_DVSInfoRequest tlvDVSInfoRequest = (TLV_V_DVSInfoRequest) ConnectionManager
							.getConnection("conn1").retrieveResultItem(
					...
					if (maxChannelNumber == 1) {
						if (isWideScreen()) {
							channelOne.setBackgroundDrawable(getResources()
									.getDrawable(R.drawable.ws_btn_number1));
							...
							channelOne.setBackgroundDrawable(getResources()
									.getDrawable(R.drawable.control_number1));
							channelTwo.setClickable(false);
							channelTwo
									.setBackgroundDrawable(getResources()
											.getDrawable(
		...
		pi = PendingIntent.getBroadcast(this, 0, new Intent(MainActivity.this,
				CheckConnectionBroadcast.class), Intent.FLAG_ACTIVITY_NEW_TASK);
		am.setInexactRepeating(AlarmManager.RTC_WAKEUP,
				System.currentTimeMillis(), 180 * 1000, pi);
	}
	...
	private OnClickListener switchChannel = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			TLV_V_DVSInfoRequest tlvDVSInfoRequest = (TLV_V_DVSInfoRequest) ConnectionManager
					.getConnection("conn1").retrieveResultItem(
							TLV_T_Command.TLV_T_DVS_INFO_REQUEST);
			TLV_V_ChannelResponse tlvChannelResponse = (TLV_V_ChannelResponse) ConnectionManager
					.getConnection("conn1").retrieveResultItem(
							TLV_T_Command.TLV_T_CHANNLE_ANSWER);
			if (tlvDVSInfoRequest != null && !isCloudControl) {
				...
				if (dvrDevice.getChannelNumber() != null) {
					maxChannelNumber = Integer.parseInt(dvrDevice
							.getChannelNumber());
				}
			...
			if (tlvChannelResponse != null) {
				...
				case R.id.btn_number_1:
					...
					if (maxChannelNumber >= page * 4 + 1) {
						...
						favouriteControlService.setLastChannel(filePath,
								String.valueOf(page * 4 + 1));
					}
					break;
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case SearchableDeviceListActivity.DDNS_ERR_NULLSETTING:
			Toast.makeText(this, getString(R.string.IDS_PROMPT_NULLSETTING),
					Toast.LENGTH_LONG).show();
			break;
		}
		...
		if (isPlay) { // 播放状态
			if (isWideScreen()) {
				playBtn.setBackgroundDrawable(getResources().getDrawable(
						R.drawable.ws_linear_play));
			} else {
				playBtn.setBackgroundDrawable(getResources().getDrawable(
						R.drawable.linear_play));
			}
